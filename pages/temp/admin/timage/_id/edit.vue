<template lang="pug">
.page-temp-admin-timage-edit
  TempBreadcrumb(top)
    ElBreadcrumbItem(:to="{ path: '/temp/admin' }") Admin
    ElBreadcrumbItem(:to="{ path: route('base') }") {{ type_label }}
    ElBreadcrumbItem Edit {{ type_label }}
  .page-temp-admin-timage-edit__content
    h1 Edit {{ type_label }}
    ElForm.page-temp-admin-timage-edit__form
      .page-temp-admin-timage-edit__operations
        EditorInputSwitch.page-temp-admin-timage-edit__status(v-model="values.status", label="Status", border, :on="1", :off="0")
        ElButton(type="primary", @click="onSave") Save
        ElButton(type="danger", @click="onDelete") Delete
      .page-temp-admin-timage-edit__grid
        .page-temp-admin-timage-edit__fields
        ...
        .page-temp-admin-timage-edit__preview
          TempImage.page-temp-admin-timage-edit__image(:src="values.src", frame="2em", :mutate="getValues(['top', 'left', 'zoom', 'rotate', 'fit'])")
</template>
