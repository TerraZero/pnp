<template lang="pug">
.page-temp-admin-tslideshow-edit
  TempBreadcrumb(top)
    ElBreadcrumbItem(:to="{ path: '/temp/admin' }") Admin
    ElBreadcrumbItem(:to="{ path: route('base') }") {{ type_label }}
    ElBreadcrumbItem Edit {{ type_label }}
  .page-temp-admin-tslideshow-edit__content
    h1 Edit {{ type_label }}
    ElForm.page-temp-admin-tslideshow-edit__form
      .page-temp-admin-tslideshow-edit__operations
        EditorInputSwitch.page-temp-admin-tslideshow-edit__status(v-model="values.status", label="Status", border, :on="1", :off="0")
      ...
      TempTagsInput(v-model="values.tags", label="Tags", cat="Image")
      .page-temp-admin-tslideshow-edit__images
        ElTable.page-temp-admin-tslideshow-edit__table(:data="imagesData")
          ElTableColumn(prop="id", label="ID")
          ElTableColumn(prop="label", label="Name")
          ElTableColumn(label="Operations")
            template(#header)
              .page-temp-admin-tslideshow-edit__images-operations
                | Operations
                .page-temp-admin-tslideshow-edit__buttons
        ...
        .page-temp-admin-tslideshow-edit__image-form(v-if="selected_index !== -1")
          h2.page-temp-admin-tslideshow-edit__image-label Edit {{ selectedImage?.label() }}
          .page-temp-admin-tslideshow-edit__image-wrapper
