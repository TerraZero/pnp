<template lang="pug">
.page-temp-admin-tstate-edit
  TempBreadcrumb(top)
    ElBreadcrumbItem(:to="{ path: '/temp/admin' }") Admin
    ElBreadcrumbItem(:to="{ path: route('base') }") {{ type_label }}
    ElBreadcrumbItem Edit {{ type_label }}
  .page-temp-admin-tstate-edit__content
    h1 Edit {{ type_label }}
    ElForm.page-temp-admin-tstate-edit__form
      .page-temp-admin-tstate-edit__operations
        EditorInputSwitch.page-temp-admin-tstate-edit__status(v-model="values.status", label="Status", border, :on="1", :off="0")
