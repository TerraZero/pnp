<template lang="pug">
.page-temp-admin-tplaylist-edit
  TempBreadcrumb(top)
    ElBreadcrumbItem(:to="{ path: '/temp/admin' }") Admin
    ElBreadcrumbItem(:to="{ path: route('base') }") {{ type_label }}
    ElBreadcrumbItem Edit {{ type_label }}
  .page-temp-admin-tplaylist-edit__content
    h1 Edit {{ type_label }}
    ElForm.page-temp-admin-tplaylist-edit__form
      .page-temp-admin-tplaylist-edit__operations
        EditorInputSwitch.page-temp-admin-tplaylist-edit__status(v-model="values.status", label="Status", border, :on="1", :off="0")
      ...
      ElTable(:data="musicsData")
        ...
        ElTableColumn(label="Operations")
          template(#header)
            .page-temp-admin-tplaylist-edit__musics-operations
              | Operations
              .page-temp-admin-tplaylist-edit__buttons
